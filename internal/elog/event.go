...
// CID вычисляет content ID события
func (e *Event) CID() CID {
	h := sha256.New()

	// TID
	binary.Write(h, binary.BigEndian, uint64(e.TID))

	// Parents (отсортированы для детерминизма)
...
// UnmarshalEvent десериализует событие
