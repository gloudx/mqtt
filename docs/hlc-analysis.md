...
```
┌─────────────────────────────────────────────────────┬────────────────┐
│         Physical Time (53 бита)                     │ Random (11 бит)│
│         Timestamp в микросекундах                   │  Случайные     │
└─────────────────────────────────────────────────────┴────────────────┘
 63                                                  11 10             0
```
...
- **Биты 10-0 (11 бит)**: Random (0-2047)
  - Случайные биты для уникальности
  - Генерируются криптографически стойким генератором (crypto/rand)
...
- Не требует централизованного управления ID узлов
...
**Что делает:**
1. Инициализирует maxDiff = 1 минута
2. Устанавливает now() = текущее время системы
3. PT и LC начинаются с 0 (устанавливаются при первом вызове Now())
...
2. Сгенерировать случайные 11 бит через crypto/rand
3. Закодировать в 64-битный TID:
   TID = (c.pt << 11) | random
        └─────┬─────┘   └──┬──┘
          53 бита       11 бит
          timestamp     random
...
TID = (1701360000000000 << 11) | 1337
    = 3485497344000000000 | 1337
    = 0x306D5179D6800539
```
...
**Ограничение:**
- TID не сохраняет LC, только PT и Random биты
- Точность восстановления: до микросекунды

### Random() - Извлечение случайных бит
...
**Алгоритм:**
```
Random = TID & 0x07FF  // Биты 10-0 (11 бит)
```
...
**Назначение:**
- Создать TID для заданного времени и случайных бит
...
- from → Random = 0 (минимальный TID)
- to → Random = 2047 (0x7FF, максимальный TID)
...
**Почему Random = 0 и 2047?**
- Random = 0: минимальный возможный TID для данного времени
- Random = 2047 (0x7FF): максимальный возможный TID для данного времени (11 бит)
...
2. **Максимум событий в микросекунду:**
   - LC: 16 бит → 65536 событий/μs (для Timestamp)
   - Если превышено: потеря точности (PT увеличится на 1 μs)

3. **Вероятность коллизий TID:**
   - 11 бит random → 2048 вариантов на микросекунду
   - Вероятность коллизии при одновременной генерации: ~1/2048
   - На практике коллизии редки из-за точности до микросекунды
...
- Уникальность: timestamp + random биты (криптографически стойкий генератор)

**Пример использования:**
```go
// Создать TID для записи
tid := clk.TID()
recordKey := tid.String()  // "7ksdn8tqfmkpy"

// Использовать как ключ записи
...
4. **Использование TID vs Timestamp:**
